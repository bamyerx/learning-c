/*
 * Project 1 from Chapter 17 of King: Modify the inventory.c program of Section
 * 16.3 so that the inventory array is allocated dynamically and later 
 * reallocated when it fills up. Use malloc initially to allocate enough space
 * for an array of 10 part structures. When the array has no more room for new
 * parts, use realloc to double its size. Repeat the doubling step each time
...
void insert(void)
{
	...
	if (num_parts == max_parts) {
		p = realloc(inventory, 2 * max_parts * sizeof(struct part));
		if (p == NULL) {
