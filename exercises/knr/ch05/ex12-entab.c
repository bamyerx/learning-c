...
{
	int c, col, pending_blanks, ntabs, tabstop, m = 0, n = 0;
	...
	while (--argc > 0) {
		...
		if (*arg == '-') {
			...
			if (ntabs > 0 && tabstops[ntabs-1] >= tabstop) {
				printf("entab: tab stops must be strictly increasing\n");
				return 1;
			}
			if (ntabs == MAXTABS) {
				printf("entab: too many tab stops\n");
				return 1;
			}
	...
	if (m > 0 && ntabs > 0 && m < tabstops[ntabs-1]) {
		printf("entab: tab start column must be greater ");
