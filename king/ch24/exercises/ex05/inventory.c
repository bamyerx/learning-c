/*
 * Exercise 5 from Chapter 24 of King: In the inventory.c program, the main
 * function has a for loop that prompts the user to enter an operation code,
 * reads the code, and then calls either insert, search, update, or print. Add
 * a call of setjmp to main in such a way that a subsequent call of longjmp will
 * return to the for loop. (After the longjmp, the user will be prompted for an
