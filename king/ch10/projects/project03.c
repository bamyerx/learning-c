/* Project 3 of Chapter 10 of King: Remove the num_in_rank, num_in_suit, and
 * card_exists arrays from the poker.c program of Section 10.5. Have the program
 * store the cards in a 5 x 2 array instead. Each row of the array will 
 ...
 *               of pairs; stores the result into the external variables
 *               straight, flush, four, three, and pairs.
 */
void analyze_hand(void)
{
   int i, card, rank, suit, num_consec;
   straight = true;
   flush = true;
   four = false;
   three = false;
   ...
   for (card = 1; card < NUM_CARDS; card++) {
      ...
      suit = hand[card][SUIT];
      i = card - 1;
      for (; i >= 0 && hand[i][RANK] > rank; i--) {
         hand[i + 1][RANK] = hand[i][RANK];
         hand[i + 1][SUIT] = hand[i][SUIT];
      }
      hand[i + 1][RANK] = rank;
      hand[i + 1][SUIT] = suit;
   }
